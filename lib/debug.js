...
router.get("/debug", (req, res) => {
    ...
    return res.send(JSON.stringify(parseObject(result, req.query.exclude), null, 4));
});

router.get("/debug/rooms", (req, res) => {
    res.header("Content-Type", "application/json");
    return res.send(JSON.stringify(Object.keys(parseObject(global.jbg.rooms, req.query.exclude)), null, 4));
});
...
router.get("/debug/external-requests", (req, res) => {
    res.header("Content-Type", "application/json");
    return res.send(JSON.stringify(Object.keys(parseObject(global.jbg.externalRequests, req.query.exclude)), null, 4));
});
...
router.get("/debug/external-requests/:token", (req, res) => {
	...
	res.header("Content-Type", "application/json");
	return res.send(JSON.stringify(parseObject(externalRequest, req.query.exclude), null, 4));
});
