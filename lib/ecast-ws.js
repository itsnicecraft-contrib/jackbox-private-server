...
module.exports = function(client, request, req){
	...
	client.on('message', data => {
		...
		if(client.jbg.room && global.jbg.rooms[client.jbg.room] && utils.isJson(data)) global.jbg.rooms[client.jbg.room].messages.push({
			...
			incoming: true,
			server: 'ecast',
			from: global.jbg.rooms[client.jbg.room].getPlayerByUserId(client.jbg.userId, true),
			to: null,
			pc: null,
...
function actionEntity(c, msg){
	...
	if(!utils.checkEntityParams(type, action, msg.params)){
		...
		if(accept){
			if(accept.length){
				if(accept.length.min !== undefined && params.val < accept.length.min){
				...
				if(accept.length.max !== undefined && params.val > accept.length.max){
...
const opcodes = {
	...
	'artifact/create': (c, msg) => {
		if(!c.jbg.room){
			c.sendError(msg.seq, 2024);
			return false;
		}else if(!msg.params.key || !msg.params.blob || !msg.params.categoryId || !global.jbg.artifactCategories[msg.params.categoryId]){
			c.sendError(msg.seq, 2004);
			return false;
		}
		global.jbg.rooms[c.jbg.room].sendToAll({
			opcode: "artifact",
			result: {
