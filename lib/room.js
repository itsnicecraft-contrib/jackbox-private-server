...
class Room {
	constructor(params, server){
		...
		else this.roomId = utils.make('room');
	...
	getEntityBody(key, textMapMakeText, textMapIncludeNodes){
		let content = {};
	...
	connect(wsId, userId, name, role, server, replaySince){
		...
		if(reconnect){
			if(role == 'host'){
				...
				this.clients[this.getPlayerByUserId(userId, true)].wsId = wsId;
				this.clients[this.getPlayerByUserId(userId, true)].connected = true;
				this.clients[this.getPlayerByUserId(userId, true)].server = server;
				profileId = this.clients[this.getPlayerByUserId(userId, true)].profileId;
			}
		...
		if(role == 'player' && this.server == 'blobcast'){
	...
	create(type, key, acl, content){
		...
		if(type == 'number'){
			...
			this.entities[key].playersTexts = [];
			...
			content.choices && content.choices.forEach(choice => {
				this.entities[key].choices[choice] = 0;
			});
	...
	increment(key, content = {}){
		...
		if(this.entities[key].type == 'number'){
			...
			this.entities[key].choices[content.vote] += 1 * content.times;
		}else if(this.entities[key].type == 'audience/g-counter'){
			this.entities[key].count += 1 * content.times;
		}else if(this.entities[key].type == 'audience/pn-counter'){
			this.entities[key].count += 1 * content.times;
		}else{
			return false;
	...
	sync(key, content, playerId){
		if(!this.entities[key]) return null;
		// parse that fuck base64 data
		this.entities[key].playersTexts.push({
			msg: content.msg,
			from: playerId
		});
