	...
	client.sendMsg = msg => {
		...
		if(client.jbg.room && global.jbg.rooms[client.jbg.room]){
			...
			global.jbg.rooms[client.jbg.room].messages.push({
				...
				server: 'blobcast',
				from: null,
				to: global.jbg.rooms[client.jbg.room].getPlayerByUserId(client.jbg.userId, true),
		...
		if(parts[0] == "0"){
			...
			if(msg.args.constructor == Array){
				msg.args.forEach(arg => {
					...
					if(actions[action]) actions[action](client, arg);
				});
			}else{
		...
		if(msg.joinType == 'player'){
			...
			if(global.jbg.rooms[msg.roomId].isTwitchLocked() && !twitchLoggedIn){
				...
				if(joined){
					...
					global.jbg.rooms[msg.roomId].sendMsg(profileId, {
						opcode: 'object',
